...
exports.getBloodBankLocation = (req,res)=>{
    let sqlQuery = "SELECT DISTINCT blood_bank_location,blood_group FROM employee_general_detail where donate = 1 and blood_bank_location != '' group by  blood_group";
    con.query(sqlQuery,[], function (err, result, fields) {
        if (err){
            response(res,400,err,{'status':'failed'});
        }else{
...
function getEmpDetOutFun(employee_id) {
    return new Promise(function(resolve,reject) {
        requestify.get('https://emss.mahindra.com/sap/bc/zzhr_chk_sf_emp?sap-client=500&empid='+employee_id).then(function(result) {
            // Get the response body (JSON parsed - JSON response or jQuery object in case of XML response)
            result.getBody();
...
//Mldl
exports.mldlGetToken = (req,res) =>{
    let parameters = res.paramDecrpted;
    let employee_id = parameters.employee_id;
    requestify.get('https://ap6.salesforce.com/services/oauth2/token?grant_type=password&client_id=3MVG9e2mBbZnmM6nS2b1kONDo456PG.SPfXBsICRvM_moq034zulHVIGu_rp98HxDOVV._hRN0vkd842vRNiQ&client_secret=64AADBC47ECC54895EA3A4C9EEE46F45F2495F4541D1BF57705D2409BD500BD8&username=samal.akash@mahindra.com&password=Pitambar123@').then(function(result) {
        // Get the response body (JSON parsed - JSON response or jQuery object in case of XML response)
        result.getBody();
        // Get the response raw body
        let val = result.body;
...
exports.getMahindraHealthReport = (req,res) =>{
    console.log('getMahindraHealthReport');
    let parameters = res.paramDecrpted;
    let employee_id = parameters.employee_id?parameters.employee_id:'';
    let year = parameters.year?parameters.year:'';
    ...
    requestify.get(url).then(function(result) {
        ...
        console.log('getMahindraHealthReport res',result.getBody());
        response(res,200,'',{'status':'success','data':val});
    },err=>{
        console.log('getMahindraHealthReport res',err);
        response(res,200,'',{'status':'success','data':{}});
    });
}
//------------------------------------willing to donate --------------------------------------------------------------
exports.addUpdateWillingToDonateInfo = (req,res)=>{
    ...
    getlatLong(present_address).then(function(data){
        let latLong = data;
        let param = [employee_name,address,present_address,contact,designation,mail_id,willing_to_help,latLong.lat,latLong.lng,employee_id];
        ...
        con.query(sqlQuery,[employee_id], function (err, result, fields) {
            if (err){
                ...
                if(result.length==0){
                    sqlQuery = 'INSERT INTO willing_to_help (employee_name,address,present_address,contact,designation,mail_id,willing_to_help,latitude,longitude,employee_id) VALUES (?,?,?,?,?,?,?,?,?,?)' ;
                }else{
                    sqlQuery = 'UPDATE willing_to_help set employee_name=?,address=?,present_address=?,contact=?,designation=?,mail_id=?,willing_to_help=?,latitude=?,longitude=? WHERE employee_id = ?' ; 
                }
                con.query(sqlQuery,param, function (err, result, fields) {
                    if (err){
                        response(res,400,err,{'status':'failed'});
...
exports.gethelpers = (req,res) =>{
    ...
    var lat=parameters[key].lat;
    var long=parameters[key].lng;
    var radiusInKm=5;
            ...
            if (err){
                ...
                if(result.length>0){
                    ...
                    requestify.get('https://emss.mahindra.com/sap/bc/zzhr_chk_sf_emp?sap-client=500&empid='+employee_id).then(function(result) {
                        ...
                        let User = employee_id+'@Mahindra';
                        requestify.request(url, {method: 'GET',auth: {username: User,password: employee_id}}).then(function(result) {
                            result.getBody();
...
function getlatLong(address){
    return new Promise(function(resolve,reject){
        let latLongUrl = 'https://maps.googleapis.com/maps/api/geocode/json?address='+address+'&key=AIzaSyCpFuGsz_ZjM5sN-Sjaro_71dcT8nejnXw';
