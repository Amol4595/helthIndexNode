...
exports.getGenDetails = (req,res)=>{
    let parameters = res.paramDecrpted;
    if(parameters.employee_id){
        let fields = "* ";
        let select = "SELECT ";
        let from = " from  employee_general_detail WHERE 1=1";
        let where = "";
        let limit = "";
        let offset = "";
        let sort = " ";
...
function responseNoEncrypt(res,resStatus,err,resData){
    res.status(resStatus).json({'res':resData});
    if(resStatus==400){
        throw err;
    }
}
    ...
    if(auth[1]==''||auth[1]==undefined){
        ...
        decryptDataString(auth[2]).then((employee_id)=>{
            jwt.verify(auth[1], privateKey, function(err, decoded) {
                ...
                if((decoded!=undefined&&decoded.employee_id==employee_id)||(auth[1]==123)){
                    verifyTokenWithDb(employee_id,auth[1]).then((valid)=>{
						if(auth[1]==123){valid=true;}
                        if(valid){
...
exports.addUpdateWillingToDonateInfo = (req,res)=>{
    ...
    let tableName = 'willing_to_help';
	
    if(req.headers.origin.includes("themahindraway")){
        tableName = 'willing_to_help_vapt';
    }else{
        tableName = 'willing_to_help';
    }
    getlatLong(present_address).then(function(data){
        ...
        let sqlQuery = 'SELECT * from '+tableName+' where employee_id = ?' ;
        con.query(sqlQuery,[employee_id], function (err, result, fields) {
            if (err){
                response(res,400,err,{'status':'failed'});
            }else{
                if(result.length==0){
                    sqlQuery = 'INSERT INTO '+tableName+' (employee_name,address,present_address,contact,designation,mail_id,nameofcostcenter,willing_to_help,latitude,longitude,employee_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)' ;
                }else{
                    sqlQuery = 'UPDATE '+tableName+' set employee_name=?,address=?,present_address=?,contact=?,designation=?,mail_id=?,nameofcostcenter=?,willing_to_help=?,latitude=?,longitude=? WHERE employee_id = ?' ; 
                }
                con.query(sqlQuery,param, function (err, result, fields) {
                    if (err){
                        response(res,400,err,{'status':'failed'});
...
exports.gethelpers = (req,res) =>{
    let tableName = 'willing_to_help';
	console.log('req.headers.origin.includes("themahindraway")',req.headers.origin,req.headers.origin.includes("themahindraway"));
    if(req.headers.origin.includes("themahindraway")){
        tableName = 'willing_to_help_vapt';
    }else{
        tableName = 'willing_to_help';
    ...
    let sqlQuery = select+fields+from+where+limit+offset;
    //console.log('getDonors',sqlQuery);
    con.query(sqlQuery, function (err, result, fields) {
        if (err){
            response(res,400,err,{'status':'failed'});
        }else{
...
exports.getEmpDetOutWillingToHelpNew = (req,res) =>{
    ...
    con.query(sqlQuery,[employee_id], function (err, result, fields) {
        if (err){
            ...
            requestify.get(SapUserInfoUrl).then(function(SapUserInfoUrlresult) {
                SapUserInfoUrlresult.getBody();
                var val1 = JSON.parse(SapUserInfoUrlresult.body);
                if(result.length>0){
                    requestify.request(Sapurl, {method: 'GET',auth: {username: SapUser,password: SapPass}}).then(function(Addressresult) {
                        ...
                        val = val.d.results[0];
                        let state = val?val.stateNav.picklistLabels.results[0].label:'';
                        let country = val?val.countryNav.territoryName:'';
                        let city = val?val.cityNav.picklistLabels.results[0].label:'';
                        let pincode = val?val.zipCode:'';
                        let address = val?val.address1:'';
                        address += val?val.address2?', '+val.address2:'':'';
                        address += val?val.address3?', '+val.address3:'':'';
                        address += val?val.address4?', '+val.address4:'':'';
                        address += val?val.address5?', '+val.address5:'':'';
                        address += val?val.address6?', '+val.address6:'':'';
                        address += val?val.address7?', '+val.address7:'':'';
                        address += val?val.address8?', '+val.address8:'':'';
                        address += val?val.address9?', '+val.address9:'':'';
                        ...
                        result[0].empSfFlag = val1.empSfFlag;
                        result[0].nameofcostcenter = val1.nameofcostcenter;
                        response(res,200,'',{'status':'success','data':result[0]});
                    },err=>{
                        response(res,200,'',{'status':'success','data':{}});
                ...
                response(res,200,'',{'status':'success1','data':{}});
            });
        }
    });
}
...
exports.emssEmpData = (req,res)=>{
    var employee_id = req.query.id?+req.query.id:'';
	...
	requestify.get('https://emss.mahindra.com/sap/bc/zzhr_chk_sf_emp?sap-client=500&empid='+employee_id).then(function(result) {
		...
		// Get the response raw body
		let val = result.body;
		responseNoEncrypt(res,200,'',{'status':'success','data':val});
	},err=>{
		responseNoEncrypt(res,200,'',{'status':'success','data':{}});
	});
}
